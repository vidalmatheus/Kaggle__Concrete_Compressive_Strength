...
tangente <- c("tan(<var>)","tan(<var><op><exp>)")
tangenteh <- c("tanh(<var>)","tanh(<var><op><exp>)")
logaritmo <- c("log(<var>)","log(<var><op><exp>)")
exp_natural <- c("exp(<var>)","exp(<var><op><exp>)")
exponencial <- c("<exp>^(<var>)","<exp>^(<var><op><exp>)")
raiz <- c("sqrt(<var>)","sqrt(<var><op><exp>)")
modulo <- c("abs(<var>)","abs(<var><op><exp>)")
incremento <- c("<var>+1","<var>+1<op><exp>")
decremento <- c("<var>-1","<var>-1<op><exp>")
constante <- c("0.<dig><dig><dig>",rep("0.<dig><dig><dig><op><exp>",5))
dig <- c("1","2","3","4","5","6","7","8","9")

P <- list(c(basicas,variavel,exp_natural,constante),op,var,dig)
#P <- list(c(basicas,seno,cosseno,senoh,cossenoh,tangente,tangenteh,modulo,incremento,decremento,logaritmo,exp_natural,raiz,constante),op,var,dig)
...
max_subst <- 100
fator_cruzamento <- 4
C <- matrix(sample(max_gene,cromo_size*cromo_quant,replace = TRUE),cromo_quant,cromo_size) # Ci = C[i,]
...
para.cruzar <- which.minn(erroQuadraticoMedio[validos],n=melhores) # pega os melhores 
para.cruzar
# for (i in 1:melhores){
#   sorteio <- rep(NA,melhores)
...
for (g in 1:N){
  ...
  C[which(!c(1:cromo_quant) %in% para.cruzar),] <- sample(max_gene,cromo_size)
  ...
  if (raizErroQuadraticoMedio<raizErroQuadraticoMedioMin){
    ...
    derivacao_vencedora <- derivacao[vencedor]
    populacao_vencedora <- C
    cat("Achou pto de mínimo: ",raizErroQuadraticoMedio,"\n")
  }
}
```
