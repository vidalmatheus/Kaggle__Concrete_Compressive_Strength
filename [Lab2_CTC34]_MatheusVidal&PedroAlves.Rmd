...
# Não Terminais
V <- c("<exp>","<op>","<var>","<sin>","<cos>","<log>","<e>","<^>","<abs>")
...
P <- list(c("<exp><op><exp>","<var>",seno,cosseno,logaritmo,exp_natural,exponencial,modulo),op,var)
...
for (i in 1:cromo_quant){
  ...
  while (is.na(erroQuadraticoMedio[pos[1]]) || is.na(erroQuadraticoMedio[pos[2]]) || erroQuadraticoMedio[pos[1]]==Inf || erroQuadraticoMedio[pos[2]]==Inf){
    pos <- sample(cromo_quant,2) # sorteio dois crommossomos  
  }
  #sorteio[pos[i,]] <- pos[i,]
  sorteio[pos] <- pos
