...
const_quant <- 250000
amostra <- 250000
const <- c(const, runif(const_quant,min = -max_const,max=max_const))
...
var <- rep(c("x","y","z","u","v","w","p","q"),10000)
var<-c(var,const)
sigma <- c(op,var)
...
# Produções
basicas <- rep(c("<exp><op><exp>"),1200)
variavel <- rep(c("<var>"),900)
seno <- c("sin(<var>)",rep(c("sin(<var><op><exp>)"),9))
cosseno <- c("cos(<var>)",rep(c("cos(<var><op><exp>)"),9))
logaritmo <- c("log(<var>)",rep(c("log(<var><op><exp>)"),9))
exp_natural <- c("exp(<var>)",rep(c("exp(<var><op><exp>)"),9))
exponencial <- c("<exp>^(<var>)",rep(c("<exp>^(<var><op><exp>)"),9))
modulo <- c("abs(<var>)",rep(c("abs(<var><op><exp>)"),9))




P <- list(c(basicas,seno,cosseno,logaritmo,exp_natural,exponencial,modulo),op,var)
...
sorteados <- order(erroQuadraticoMedio[validos])[1:50]
for (i in 51:cromo_quant){
